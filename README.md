...
본 프로젝트는 복잡한 빌드 도구나 웹 서버 환경 없이, 파일들을 로컬에 두고 브라우저에서 곧바로 실행할 수 있도록 정갈하게 폴더를 구조화해 사용합니다.

├── index.html         # 웹사이트 소스 코드 (HTML, CSS, JS 통합)
...
주의: 사진과 비디오가 정상적으로 출력되도록 반드시 index.html이 있는 경로에 assets 폴더를 만들고 지정된 파일명으로 자산들을 위치시켜 주십시오.

시작하기 (Getting Started)

이 저장소의 모든 파일을 로컬 환경의 동일한 디렉토리에 다운로드합니다.

assets 폴더를 생성하고 필요한 미디어 파일들을 저장합니다.

shibuya_video.mp4

bateaux_mouches.JPG

bateaux_mouches_video.mov

index.html 파일을 마우스 더블 클릭하여 크롬, 사파리, 엣지 등의 브라우저에서 즉시 실행합니다.

상단의 '빗소리 앰비언스 ON' 버튼을 클릭하여 스피커 너머로 잔잔하게 흐르는 실시간 빗소리를 함께 감상해 보세요.
